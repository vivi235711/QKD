function [rate, keyl] = error_correct_1(size,error_rate,repeat)
...
for n = 1:repeat-1    
    sequence = shuffle(size);
    [Sa, Sb] = compare_and_correct2(Sa, Sb, sequence, block_size, size)
...
function [Sa, Sb] = compare_and_correct2(Sa, Sb, sequence, block_size, size)
parities_a = zeros(1,ceil(size/block_size));
parities_b = zeros(1,ceil(size/block_size));
for n = 1:length(parities_a)
    if(n ~= length(parities_b))
...
function [sequence] = shuffle(size)
sequence = 1:size;
sequence = sequence(randperm(size));
end
